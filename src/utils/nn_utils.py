...
sys.path.insert(0, '../')
from src.evaluation.utils import map_prop_to_idx
# from data.mol_bpe import PIECE_CONNECT_NUM


def to_one_hot(idx, size):                 # 独热编码
...
def pyg_batch_to_tensors(batch, vocab):
    ...
    add_edge_mask = []  # 由于 BCELOS 没有ignore_idx选项，因此需要掩码（所选为 1）
    attr_dim = data_list[0].edge_attr.shape[1]
    device = data_list[0].x.device
    na = len(vocab)  # 原子类型数

    for i, data in enumerate(data_list):
        x.append(data.x[:, :na])  # 仅保留原子类型
