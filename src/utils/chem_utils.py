...
Bond_List = [None, BondType.SINGLE, BondType.DOUBLE, BondType.TRIPLE]  # aromatic bonds are shits 键的类型：单键，双键，三键


class GeneralVocab:        # 一般词汇
...
def data2molecule(vocab, data, sanitize=True):
    ...
    for atom_idx in data.x:  # add atoms
        if not isinstance(atom_idx, int):  # one-hot form
            atom_idx = torch.argmax(atom_idx[:len(vocab)])     # argmax()返回指定维度最大值的序号
...
def get_base64(mol):
    return Chem.RDKFingerprint(mol).ToBase64()    # 将拓扑指纹转换为 base-64 编码形式


def dfs_order(mol, root):
    """返回 按dfs深度优先算法排序的原子列表和 IDX2Order映射字典"""
    stack = [root]
    visited = {}
    order_list = []
    idx2order = {}
...
def bfs_order(mol, root):      # 广度优先算法排序
    queue = Queue()
    queue.put(root)
    visited = {}
    order_list = []
    ...
    while not queue.empty():
        ...
        for nei in atom.GetNeighbors():
            ...
            if idx not in visited:
                visited[idx] = True
                neis.append(idx)
        neis.sort(key=lambda i: id2flow[i])  # 流量值越低的节点优先级越高
        for idx in neis:
...
def get_random_submol(mol):  # 使用BFS顺序并随机掉落1-5个原子
