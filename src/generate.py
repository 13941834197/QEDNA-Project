...
def exp_prop(args):
    """首先生成受所选属性约束的10000个分子。然后将生成的smiles保存到output_path。输出成功率存到日志log。
    props：str 列表，其中包含所选属性的名称 logp or qed
    n_sample：要生成的分子数
    output_path：保存输出分子的路径 """
    model = load_model(args.ckpt, args.gpus)
    model.eval()      # 将模型设置为评估模式，确保了在评估或测试阶段模型的稳定性和准确性，通过关闭某些层的行为变化和关闭梯度计算来提高评估效率
    ...
    # predict properties and success rate based on predict properties
...
def constraint_optim(args):
    """仅适用于惩罚性 LogP 约束优化的实验"""
    # 从训练集中找到 n 个具有最小 logp 的分子
    model = load_model(args.ckpt, args.gpus)
    model.eval()
...
def parse():
    ...
    parser.add_argument('--add_edge_th', type=float, default=0.5,
                        help='添加边的阈值')
    # 解码策略：采用Temperature采样
    parser.add_argument('--temperature', type=float, default=0.8,
                        help='Decoding temperature')
    # Beam Search集束搜索
    parser.add_argument('--beam', type=int, default=5,
                        help='为了一个潜在向量z生成的分子数')
    parser.add_argument('--lr', type=float, default=0.1,
                        help='优化学习率')
    parser.add_argument('--max_iter', type=int, default=20,
                        help='最大迭代步数')
    # 模型采用了提前停止策略，参数patience表示了epochs数量，当在这个过程性能无提升时会停止训练
...
def main(args):
    setup_seed(args.seed)  # 注释掉固定复现结果处
    if args.eval:                # 进行属性优化
        exp_prop(args)
        return
    if args.constraint_optim:    # 进行受限属性优化
