...
from src.modules.vae_piece_decoder import VAEPieceDecoder
from src.modules.predictor import Predictor
from src.data.bpe_dataset import BPEMolDataset, get_dataloader
from src.evaluation.utils import get_normalized_property_scores, PROPS
...
class PSVAEModel(pl.LightningModule):
    ...
    def forward(self, batch, return_accu=False):  # 向前算法
        # 有关批处理的定义，请参阅data.bpe_dataset
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss = self.weighted_loss(pred_loss, rec_losses[0],
                                  rec_losses[-1])  # first is overall rec loss, last is kl loss
