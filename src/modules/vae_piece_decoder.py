...
class VAEPieceDecoder(nn.Module):
    ...
    def forward(self, x, x_pieces, x_pos, edge_index, edge_attr, pieces, conds, edge_select, golden_edge,
                return_accu=False):
        # x: [batch_size, N, dim_in]
        # edge_index 和 edge_attr 不包括要预测的边缘
        # pieces: [batch_size, seq_len] in idx form
        # conds: [batch_size, cond_dim]
        # edge_select: [batch_size, N, N] 指示要预测的边
        # golden_egdes: [E], 仅包括要预测的边缘
        ...
        # edge prediction
        # [batch_size, N, N, node_dim], 第二个 N 包含相同的节点嵌入
        src_embedding = torch.repeat_interleave(node_embedding, node_num, dim=1).view(batch_size, node_num, node_num,
                                                                                      -1)
        ...
        dst_embedding = torch.repeat_interleave(node_embedding, node_num, dim=0).view(batch_size, node_num, node_num,
                                                                                      -1)
        ...
        if return_accu:
            ...
            piece_accu = ((torch.argmax(pred_piece,
                                        dim=-1) == gold_piece) & not_pad).sum().item() / not_pad.sum().item()
    ...
    def inference(self, z, max_atom_num, add_edge_th, temperature):
        ...
        hidden = self.latent_to_rnn_hidden(z).unsqueeze(0)  # 解压 1 层 RNN
        ...
        while cur_piece_id != self.tokenizer.end_idx():
            ...
            if cur_atom_num > max_atom_num:  # 最后一个仍将被丢弃
                break
        ...
        latent_repeat = latent_repeat[edge_select]
        edge_pred_in = torch.cat([src_embedding, dst_embedding, latent_repeat], dim=-1)
        if edge_pred_in.shape[0]:  # 也许只生成一个片段 - >不需要预测边
            pred_edge = self.edge_predictor(edge_pred_in)  # [E, num_edge_type]
            ...
            for i in sorted_idx:
                ...
                if valence_check(atoms[begin], atoms[end], edge_sets[begin],
                    ...
                    if bid1 != bid2:
                        gid = aid2gid[begin]
                        for aid in aid2bid:  # 将 block1 中的所有原子重定向到 block2
                            if aid2bid[aid] == bid1:
                                aid2bid[aid] = bid2
                        block_atom_cnt[bid2] += block_atom_cnt[bid1]

        # 删除孤立的部分，查找具有最大原子数的连接块
    ...
    def inference_constraint(self, z, max_atom_num, add_edge_th, temperature, constraint_mol):
        ...
        for begin, end in inter_group_edges:
            ...
            bid1, bid2 = aid2bid[begin], aid2bid[end]  # 修改连接块
            if bid1 != bid2:
                gid = aid2gid[begin]
